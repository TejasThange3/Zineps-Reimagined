...
for (const [route, [title, description]] of Object.entries(pages)) {
  const html = shell(route, title, description);

  if (route === "/") {
    await fs.writeFile("dist/index.html", html);
  } else {
    /* Static hosts disagree about how to resolve an extensionless path. Some
       look for <route>/index.html, others for <route>.html. Writing both
       means the same dist/ works on Vercel, Netlify, Cloudflare Pages, S3 and
       a plain file server, without a rewrite rule. */
